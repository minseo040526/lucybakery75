...
def normalize_columns(df, is_drink=False):
    ...
    if df["price"].isnull().any(): 
        st.error("price에 숫자가 아닌 값이 있습니다."); st.stop()
    df = df.reset_index(drop=True)
    df["type"] = "drink" if is_drink else "bakery"
    df["item_id"] = [("D" if is_drink else "B")+f"{i+1:04d}" for i in range(len(df))]
    return df[["item_id","name","type","category","price"]] if is_drink else df[["item_id","name","type","price"] + (["category"] if "category" in df.columns else [])]
...
with st.expander("로그인/가입(선택)"):
    ...
    with colA:
        phone_last4 = st.text_input("휴대폰 뒷 4자리", max_chars=4)
    with colB:
        pw6 = st.text_input("비밀번호(6자리)", max_chars=6, type="password")
    with colC:
        name_opt = st.text_input("이름(선택)")
    ...
    if c1.button("로그인/자동가입"):
        ...
        if len(m)==1:
            ...
            if not phone_last4 or not pw6:
                st.warning("뒷4자리/비번 입력 후 다시 눌러주세요.")
            else:
                ...
                st.success("자동 가입 후 로그인되었습니다.")
    if c2.button("게스트 주문"):
        st.session_state.user = {"user_id":"GUEST","name":"게스트","phone_last4":"","pw6":""}
        st.success("게스트로 주문합니다(쿠폰 미지원).")

user = st.session_state.user
if user is None:
    st.info("게스트 주문을 누르시거나, 로그인/자동가입을 눌러 주세요.")
    st.stop()
...
with tab_reco:
    ...
    if st.button("추천 보기", type="primary", use_container_width=True):
        ...
        if drinks.empty:
            st.warning("해당 카테고리의 음료가 없습니다."); st.stop()
        ...
        if mode=="총예산":
            ...
            total_budget = None
            per_budget = int(budget_val)

        # 베이커리 조합 만들기(상위 10개에서 n_bakery개 조합)
        bakery_pool = bakery.head(max(10, n_bakery))
        if n_bakery==0:
            bakery_combos = [[]]
        else:
            pool_iter = bakery_pool.itertuples(index=False)
            bakery_combos = []
            for combo in itertools.combinations(list(pool_iter), n_bakery):
                bakery_combos.append([{col:getattr(c,col) for col in bakery.columns}] for c in [])

        # 위 조합 생성 수정(위 comprehension 실수 방지)
        bakery_combos = []
        if n_bakery==0:
            bakery_combos = [[]]
        else:
            ...
            for combo in itertools.combinations(pool_list, n_bakery):
                items = []
                for c in combo:
                    d = {col: getattr(c, col) for col in bakery.columns}
                    items.append(d)
                bakery_combos.append(items)

        # 음료는 1인 1잔 기준으로 대표 1종만 고르고 나머진 동일종으로 가정(간단화)
        drink_candidates = drinks.head(6).to_dict("records")  # 상위 6개 후보
        results = []
        for d in drink_candidates:
            drink_cost_per = d["price"]  # 1인당 음료 1잔
            for bset in bakery_combos:
                per_price = drink_cost_per  # 1인 기준(빵은 1인 기준으로 환산하지 않음)
        ...
        if not results:
            st.warning("예산 조건에 맞는 조합이 없습니다. 금액을 조정해 주세요."); st.stop()

        # 정렬: 총액 오름차순 → 1인 가격 오름차순
    ...
    if len(st.session_state.cart)==0:
        ...
        if cB.button("주문 완료(이메일 알림)"):
            ...
            if ok:
                ...
                st.success(f"주문 접수되었습니다! #{oid} 이메일 발송 완료")
                st.session_state.cart = []; st.rerun()
            else:
                idx2 = orders[orders["order_id"]==oid].index
                if len(idx2)==1:
